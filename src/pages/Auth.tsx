...
const Auth = () => {
  ...
  const handleGoogle = async () => {
    ...
    const result = await lovable.auth.signInWithOAuth("google", {
      redirect_uri: `${window.location.origin}/admin/portafolio`,
    });
    if (result.error) {
      setLoading(false);
    ...
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <main className="flex-1 flex items-center justify-center pt-32 pb-20 px-6">
        <motion.div
          ...
          className="w-full max-w-md bg-card border border-border rounded-2xl shadow-elegant p-8 md:p-10"
        >
          <div className="flex flex-col items-center text-center mb-8">
            ...
            <h1 className="font-display font-black text-2xl text-foreground">Acceso Admin</h1>
          ...
          <form onSubmit={handleEmailLogin} className="space-y-4">
            ...
            <Button type="submit" disabled={loading} className="w-full bg-action hover:bg-action-glow text-action-foreground">
